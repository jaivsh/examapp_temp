...
class QuizResultFragment : Fragment(R.layout.fragment_quizresult) {
    ...
    private fun setUI(view: View) {
        ...
        val testData = Constants.curTest

        val testHistoryData = TestHistoryData(testData.id,testData.questions,Calendar.getInstance().timeInMillis.toString(),Constants.curContentData.name)

        mainViewModel.updateTestData(testHistoryData)

        val testHistoryList = Constants.curUserData.testHistory.toMutableList()
        testHistoryList.add(testHistoryData)
        Constants.curUserData.apply {
            this.testHistory = testHistoryList
        }
